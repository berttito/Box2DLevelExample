...
void Gameobject::render(sf::RenderWindow & renderer)
{
	...
	for (b2Body * body = scene.getPhysicsWorld().GetBodyList(); body != nullptr; body = body->GetNext())
	{
		...
		for (b2Fixture * fixture = body->GetFixtureList(); fixture != nullptr; fixture = fixture->GetNext())
		{
			...
			if (shape_type == b2Shape::e_circle)
			{
				...
				//b2mul multiplica matrices
				//funcion q convierte la posicion de box2d en la posicion de sfml
				shape.setPosition(box2d_position_to_sfml_position(b2Mul(body_transform, center), window_height) - sf::Vector2f(radius, radius));
				...
				if (shape_type == b2Shape::e_edge)
				{
					...
					sf::Vertex line[] =
					{
						sf::Vertex(box2d_position_to_sfml_position(start, window_height), sf::Color::Green),
						sf::Vertex(box2d_position_to_sfml_position(end, window_height),sf::Color::Green),
					};
